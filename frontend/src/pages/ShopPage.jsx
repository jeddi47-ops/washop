...
export default function ShopPage() {
  ...
  if (!vendor) return <div className="min-h-screen pt-20 flex items-center justify-center text-gray-400">Boutique non trouvee</div>;
  ...
  return (
    <div className="min-h-screen pt-16 pb-10 animate-fade-in" data-testid="shop-page">
      {/* Banner */}
      <div className="bg-gradient-to-b from-black to-[#0a2a1a] py-12 px-4">
            ...
            {vReviews.length === 0 ? <p className="text-gray-500 text-sm">Aucun avis.</p> :
              vReviews.map((r, i) => (
                <div key={i} className="glass p-4">
                  <div className="flex items-center gap-1 mb-2">{Array(5).fill(0).map((_, j) => <Star key={j} className={`w-3.5 h-3.5 ${j < r.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-700'}`} />)}</div>
