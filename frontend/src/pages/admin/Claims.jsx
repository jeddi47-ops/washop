...
import { ChevronLeft, MessageSquare, UserPlus, Send, Loader2 } from 'lucide-react';
...
export default function AdminClaims() {
  ...
  const fetch = useCallback(async () => {
    ...
    try { const { data } = await claims.list(params); setItems(data.data || []); setTotal(data.total || 0); } catch {}
    setLoading(false);
  }, [filter]);
  ...
  return (
    <div className="min-h-screen pt-20 pb-10 px-4 animate-fade-in" data-testid="admin-claims">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-900">Reclamations ({total})</h1>
        </div>
        ...
        items.length === 0 ? <div className="text-center py-20 text-gray-400"><MessageSquare className="w-16 h-16 mx-auto mb-4 opacity-30" /><p>Aucune reclamation</p></div> :
        <div className="space-y-2">
          {items.map(c => (
            <Link key={c.id} to={`/admin/claims/${c.id}`} className="glass p-4 block hover:border-[#25D366] hover:shadow-md transition">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-400">
                ...
                {c.assigned_to && <span className="text-[#25D366]">Assigne</span>}
              </div>
            </Link>
          ))}
        </div>}
...
export function AdminClaimDetail() {
  ...
  useEffect(() => {
    fetchClaim();
    usersApi.list({ role: 'employee', limit: 50 }).then(r => setEmployees(r.data.data || [])).catch(() => {});
  ...
  const assign = async (empId) => { setAssigning(true); try { await claims.assign(id, { employee_id: empId }); fetchClaim(); } catch {} setAssigning(false); };
  ...
  return (
    <div className="min-h-screen pt-20 pb-10 px-4 animate-fade-in" data-testid="admin-claim-detail">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link to="/admin/claims" className="p-2 rounded-lg hover:bg-gray-100"><ChevronLeft className="w-5 h-5 text-gray-600" /></Link>
          <div className="flex-1 min-w-0"><h1 className="text-lg font-bold text-gray-900 truncate">{claim.subject}</h1><p className="text-xs text-gray-500">{new Date(claim.created_at).toLocaleDateString()}</p></div>
        ...
        <div className="glass p-4 mb-4 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            ...
            <select onChange={e => e.target.value && assign(e.target.value)} disabled={assigning} className="!text-sm !py-1.5 flex-1" defaultValue="">
              <option value="" disabled>{claim.assigned_to ? 'Reassigner' : 'Assigner a...'}</option>
