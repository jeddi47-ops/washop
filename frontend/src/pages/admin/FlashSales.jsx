...
import { flashSalesAdmin, products as productsApi } from '../../lib/api';
import { ChevronLeft, Zap, Plus, X as XIcon, Trash2, Loader2, Pause } from 'lucide-react';
...
export default function AdminFlashSales() {
  ...
  const create = async (e) => {
    ...
    try {
      ...
      setShowForm(false); setForm({ product_id: '', discount_percentage: 20, starts_at: '', ends_at: '' }); fetch();
    ...
    <div className="min-h-screen pt-20 pb-10 px-4 animate-fade-in" data-testid="admin-flash-sales">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <button onClick={() => setShowForm(true)} className="btn-primary !py-2 !px-4 !text-sm flex items-center gap-1"><Plus className="w-4 h-4" /> Creer</button>
        </div>
        ...
        {showForm && (
          <form onSubmit={create} className="glass p-5 mb-6 space-y-3 animate-fade-in">
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">ID Produit</label><input value={form.product_id} onChange={e => setForm(f => ({ ...f, product_id: e.target.value }))} required className="!text-sm" placeholder="ID du produit" /></div>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setShowForm(false)} className="btn-secondary flex-1 !text-sm">Annuler</button>
              <button type="submit" disabled={saving} className="btn-primary flex-1 !text-sm flex items-center justify-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Creer</button>
            </div>
          </form>
        )}
