      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-6">
          {[{ v: '', l: 'Tous' }, { v: 'pending', l: 'En attente' }, { v: 'whatsapp_redirected', l: 'WhatsApp' }, { v: 'confirmed', l: 'Confirme' }, { v: 'cancelled', l: 'Annule' }].map(f => (
            <button key={f.v} onClick={() => setFilter(f.v)} className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition ${filter === f.v ? 'bg-[#25D366] text-white' : 'bg-white/5 text-gray-400'}`}>{f.l}</button>
          ))}
        </div>

        {loading && items.length === 0 ? <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-24 animate-shimmer rounded-xl" />)}</div> :
        items.length === 0 ? <div className="text-center py-20"><Package className="w-16 h-16 text-gray-700 mx-auto mb-4" /><p className="text-gray-400">Aucune commande</p></div> :
  ...
  return (
    <div className="min-h-screen pt-20 pb-10 px-4 animate-fade-in" data-testid="vendor-order-detail">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link to="/vendor/orders" className="p-2 rounded-lg hover:bg-white/5"><ChevronLeft className="w-5 h-5" /></Link>
        ...
        <div className="glass p-4 mb-4">
          ...
          <div className="flex justify-between mt-4 pt-3 border-t border-white/5"><span className="font-semibold">Total</span><span className="font-bold text-[#25D366] text-lg">{total.toFixed(2)}$</span></div>
        </div>
